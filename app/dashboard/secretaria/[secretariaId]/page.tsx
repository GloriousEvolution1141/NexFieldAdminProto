...
async function PageContent({
  ...
  paramsPromise: Promise<{ secretariaId: string }>;
}) {
  ...
  const { data: secData } = await getUsuarioById(
    supabase,
    secretariaId,
    "nombres, apellidos",
  );
  const secretariaName = secData
    ? `${secData.nombres} ${secData.apellidos}`
    : "Secretaria";
  ...
  return (
    <div className="space-y-6">
      ...
      <TecnicosData
        userId={secretariaId}
        basePath={`/dashboard/secretaria/${secretariaId}/tecnico`}
        showHeader={false}
      />
    </div>
  );
