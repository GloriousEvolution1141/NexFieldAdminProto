...
async function PageContent({ paramsPromise }: { paramsPromise: Promise<{ secretariaId: string; tecnicoId: string }> }) {
    ...
    const [secRes, tecRes] = await Promise.all([
        supabase.from("usuario").select("nombres, apellidos").eq("id", secretariaId).single(),
        supabase.from("usuario").select("nombres, apellidos").eq("id", tecnicoId).single(),
    ]);

    const secretariaName = secRes.data ? `${secRes.data.nombres} ${secRes.data.apellidos}` : "Secretaria";
    const tecnicoName = tecRes.data ? `${tecRes.data.nombres} ${tecRes.data.apellidos}` : "Técnico";
                ...
                <BreadcrumbList>
                    ...
                    <BreadcrumbItem>
                        <BreadcrumbLink asChild>
                            <Link href={`/dashboard/secretaria/${secretariaId}`}>Secretaria {secretariaName}</Link>
                        </BreadcrumbLink>
                    </BreadcrumbItem>
                    <BreadcrumbSeparator />
                    <BreadcrumbItem>
