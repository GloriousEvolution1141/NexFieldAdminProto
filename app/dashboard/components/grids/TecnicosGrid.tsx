...
export function TecnicosGrid({
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
            ...
            {filtered.map((tk) => {
                const suministroCount = Array.isArray(tk.suministros) ? tk.suministros.length : 0;
                return (
                    <Link key={tk.id} href={`${basePath}/${tk.id}`} className="block">
                        <Card className="hover:shadow-md hover:border-primary/50 transition-all cursor-pointer h-full flex flex-col">
                            <CardHeader className="pb-3">
                                ...
                                <p className="text-xs text-muted-foreground font-mono">{tk.codigoUsuario || "Sin código"}</p>
                            </CardHeader>
                            <CardContent className="pb-4 flex-1 space-y-3">
                                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                    <Wrench className="h-3.5 w-3.5" />
                                    <span>{suministroCount} suministro{suministroCount !== 1 ? "s" : ""}</span>
                                </div>
                                <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                                    <Calendar className="h-3.5 w-3.5" />
                                    <span>
                                        {new Date(tk.created_at || new Date()).toLocaleDateString("es-ES", { dateStyle: "medium" })}
                                    </span>
                                </div>
