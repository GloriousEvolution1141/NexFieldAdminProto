...
import { Eye, Maximize2, Package, Calendar, Trash2, CheckSquare } from "lucide-react";
    ...
    return (
        <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
                <CreateSuministroDialog adminOrSecretariaId={userId} tecnicoId={tecnicoId} asCard />
            ...
            <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
                <DialogContent className="sm:max-w-lg">
                    <DialogHeader>
                        <DialogTitle>
                            ¿Eliminar {selected.size} suministro{selected.size !== 1 ? "s" : ""}?
                        </DialogTitle>
                        <DialogDescription>
                            Esta acción eliminará <strong>{selected.size} suministro{selected.size !== 1 ? "s" : ""}</strong> y todas sus fotos de forma permanente. No se puede deshacer.
                        </DialogDescription>
                    </DialogHeader>
        ...
        <>
            <Card className="h-full flex flex-col">
                <CardHeader className="pb-3">
                    ...
                    <CardTitle className="text-sm mt-3 leading-snug line-clamp-2">{s.nombre}</CardTitle>
                </CardHeader>
                ...
                <CardContent className="pb-2 flex-1 space-y-2">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                        ...
                        <span>
                            {new Date(s.created_at || new Date()).toLocaleDateString("es-ES", { dateStyle: "medium" })}
                        </span>
                    </div>
                    <p className="text-xs text-muted-foreground">{fotoCount} foto{fotoCount !== 1 ? "s" : ""}</p>
                </CardContent>
                                        ...
                                        <CarouselItem key={f.id}>
                                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 rounded-lg border p-4 bg-muted/30">
                                                <div className="flex flex-col gap-2 h-[350px] lg:col-span-2">
                                                    <span className="font-semibold text-lg truncate">{f.nombre || "Sin Título"}</span>
                                                    <div className="w-full h-full bg-muted-foreground/10 rounded-md overflow-hidden flex items-center justify-center relative">
                                                        {f.direccion ? (
                                                            ...
                                                            <span className="text-xs text-muted-foreground">Sin URL de imagen</span>
                                                        )}
                                                    </div>
                                                    <span className="text-xs text-muted-foreground">
                                                        {new Date(f.created_at || new Date()).toLocaleString("es-ES", { dateStyle: "long", timeStyle: "short" })}
                                                    </span>
                                                </div>
                                                <div className="flex flex-col gap-2">
                                                    <h3 className="font-semibold text-foreground/80 border-b pb-2">Descripción y Notas</h3>
            ...
            <Dialog open={!!maxFoto} onOpenChange={(v) => { if (!v) setMaxFoto(null); }}>
