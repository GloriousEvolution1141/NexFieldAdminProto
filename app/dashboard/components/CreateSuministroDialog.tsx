...
export function CreateSuministroDialog({
  adminOrSecretariaId,
  tecnicoId,
  asCard = false,
}: {
  adminOrSecretariaId: string;
  tecnicoId: string;
  asCard?: boolean;
}) {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (arrayNombres.length === 0) {
      setIsLoading(false);
      toast.error("Datos vacíos", {
        description: "Debe proveer al menos un suministro válido.",
      });
      return;
    }
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[425px] bg-slate-200">
        <DialogHeader>
          ...
          <DialogDescription>
            Escribe o pega desde Excel los nombres de los suministros (uno por
            línea). Se asignarán automáticamente al técnico actual.
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={onSubmit} className="space-y-4 pt-4">
          <div className="space-y-2">
            ...
            <Textarea
              ...
              required
              className="min-h-[150px] resize-y bg-slate-100 "
            />
          </div>
